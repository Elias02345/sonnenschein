...
**Stand:** 2026-05-13 — **Phase 4 (PipeWire/KWin-Capture): Rollback auf Stand vor KScreen-Resolver in Arbeit.**
...
- **Erreichter Meilenstein (Phase 4)**:
  ...
  - ⏪ **Rollback-Kandidat `PENDING_ROLLBACK2`**: `pwgrab.cpp` ist auf `bf7d939` zurückgesetzt, also vor den KScreen-Resolver aus `6504268`. Damit werden sowohl der KScreen-Resolver als auch die späteren Output-Management/HDR/Pacing-Experimente aus dem Laufzeitcode entfernt. WSL2-Build grün.
  - ✅ **WSL-Build grün**: `/root/snsbuild`, `cmake --build . --target sunshine -j8`, `pwgrab.cpp` kompiliert und `sunshine-0.0.0` linkt.
- **Aktueller Blocker**: CachyOS muss nach Rollback `PENDING_ROLLBACK2` bestätigen: Stream startet wieder wie beim Stand `bf7d939`/`2d5b81a`. Das bekannte 60-Hz-Problem bleibt offen.
...
**Korrektur (2026-05-13)**: Der Maintainer hat direkt nach `501431a` klargestellt, dass `67a93e3`/`6504268` bereits die defekte Version nach dem letzten guten Stand war. Der tatsächliche Zielstand für den Laufzeitcode ist `bf7d939`/`2d5b81a`, also vor dem KScreen-Resolver. `PENDING_ROLLBACK2` setzt `pwgrab.cpp` entsprechend zurück.
...
```
PENDING_ROLLBACK2 — revert(capture): restore pre KScreen resolver capture path — 2026-05-13
...
**Auf `dev` nächster Test-Commit = `PENDING_ROLLBACK2`** (Stand 2026-05-13, vor Push). Nächster Schritt ist ausschließlich CachyOS-Validierung, dass der Stream wieder wie beim Stand `bf7d939` startet. 90-Hz/HDR-Arbeit pausiert bis diese Basis wieder bestätigt ist.
