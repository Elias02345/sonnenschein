# AGENTS.md — Lies das, bevor du irgendetwas tust

Du bist eine Codex-Instanz, die am Sonnenschein-Projekt arbeitet. Sonnenschein ist ein Linux-natives, headless Moonlight-Streaming-Backend, das Apollo (einen Sunshine-Fork) als Basis nutzt und Apollos Killer-Feature (Auto-Match Virtual Display beim Pairing) nach Linux portiert.
...
- **Kein** Mention der AGENTS.md / Sessionstart-Routine in Output an den Maintainer (er weiß das).
...
- `init` — Initialisiert eine AGENTS.md (DIESE ist die — nicht überschreiben)
- `review` — PR-Review
- `security-review` — Security-Scan auf pending Branch-Changes
- `loop` / `schedule` — automatisierte Wiederholung von Aufgaben
- `Codex-api`, `anthropic-skills:*` — für Anthropic-spezifische Workflows
